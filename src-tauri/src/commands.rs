...
/// 桌面信息:偏好 + 首次运行状态。
#[tauri::command]
pub fn get_desktop_snapshot(state: State<AppState>) -> DesktopSnapshot {
...
#[tauri::command]
pub fn confirm_and_run(app: AppHandle, action: String, state: State<AppState>) -> ActionAccepted {
    ...
    let (title, message) = match action.as_str() {
        ...
        _ => return state.run_action(&action),
    };
